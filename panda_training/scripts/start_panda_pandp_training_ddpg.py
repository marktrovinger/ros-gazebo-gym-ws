...
if __name__ == "__main__":  # noqa: C901
    ...
    pkg_path = rospack.get_path("panda_training")
    ...
    alpha = rospy.get_param("/panda_training/alpha")
    gamma = rospy.get_param("/panda_training/gamma")
    n_episodes = rospy.get_param("/panda_training/n_episodes")
    n_steps = rospy.get_param("/panda_training/n_steps")
    inference = rospy.get_param("/panda_training/inference")
    ...
    model = DDPG(
        ...
        #replay_buffer_kwargs=dict({"goal_selection_strategy": goal_selection_strategy, "n_sampled_goal": 4}),
        gamma=gamma, 
        learning_rate=alpha,
        policy_kwargs={"net_arch": net_arch, "n_critics": 1}
        )
    ...
    # Safe policy.
    model_save_dir = os.path.join(outdir, "panda_results_ddpg")
